...
import {
  getMarket, getAppealBond, getTakes, bet, unstake, closeMarket, resolve, appeal, finalize, claim,
...
export default function MarketPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 lg:px-6 py-7">
      ...
      {/* pinned sources */}
      <div className="card p-4 mt-4">
        <div className="eyebrow mb-2" style={{ color: "var(--aqua)" }}>Pinned settlement sources · frozen</div>
        ...
        <p className="mono text-[0.62rem] muted mt-2">criteria: {m.criteria}</p>
      </div>
      ...
      {address && (
        <div className="flex gap-2 flex-wrap mt-5">
          ...
          {(m.status === "SETTLED" || m.status === "REFUNDING") && <button onClick={() => run("claim", () => claim(client, id))} disabled={!!busy} className="btn">{busy === "claim" ? "Claiming…" : m.status === "SETTLED" ? "Claim winnings" : "Claim refund"}</button>}
        </div>
      )}
